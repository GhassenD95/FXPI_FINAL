...
public class ServiceEquipement extends BaseService implements IService<Equipement> {
    ...
    public void edit(Equipement equipement) throws SQLException {
        String sql = "UPDATE equipement SET nom = ?, description = ?, etat = ?, typeEquipement = ?, image_url = ?, quantite = ?, installationSportive_id = ? WHERE id = ?";
        PreparedStatement stmt = con.prepareStatement(sql);
        ...
        stmt.setString(5, equipement.getImage_url());
        stmt.setInt(6, equipement.getQuantite());
        stmt.setInt(7, equipement.getInstallationSportive().getId());
        stmt.setInt(8, equipement.getId());

        stmt.executeUpdate();
        System.out.println("Equipement " + equipement.getNom() + " modifié");
    }
