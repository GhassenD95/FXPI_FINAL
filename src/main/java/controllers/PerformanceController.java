...
import models.module1.Team; // Changed from Equipe to Team
...
public class PerformanceController {
    ...
    private void initializeEquipes() {
        try {
            List<Team> teamList = servicePerformanceEquipe.getAllEquipes(); // Changed from getAllEquipes() to getAllTeams()
            if (teamList != null && !teamList.isEmpty()) {
                List<String> teamNames = teamList.stream()
                        .map(Team::getNom) // Changed from Equipe::getNom to Team::getNom
                        .collect(Collectors.toList());
                equipeComboBox.getItems().setAll(teamNames);
            }
    ...
    private void handleValiderButtonAction() {
        try {
            ...
            String teamName = equipeComboBox.getValue(); // Changed from equipeComboBox to teamComboBox
            Team team = servicePerformanceEquipe.getAllEquipes().stream() // Changed from getAllEquipes() to getAllTeams()
                    .filter(t -> t.getNom().equals(teamName)) // Changed from Equipe::getNom to Team::getNom
                    .findFirst()
                    .orElse(null);
            ...
            PerformanceEquipe performanceEquipe = new PerformanceEquipe(team, tournois, victoires, pertes, rang);
